    ...
    if feedback and attempt_num > 1:
        ...
        if not explore_mode:
            ...
            if target_strategy["description"]:
                ...
                performance_context = ""
                if int(target_strategy["performance"]) > 4015 and int(target_strategy["performance"]) < 4025:
                    performance_context = (
                        "\nIMPORTANT: This strategy performed at exactly €4020.59, which means it never executed any trades "
                        "or had a net zero impact (we always finish with €4020.59 if we do nothing). The strategy's decision logic is likely "
