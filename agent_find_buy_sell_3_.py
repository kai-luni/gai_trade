...
def generate_trading_strategy(feedback: str = "", attempt_num: int = 1, max_attempts: int = 3, previous_errors: list = None, previous_code: str = None, debug: bool = False) -> str:
    ...
    mode_description = "EXPLORATION" if (attempt_num % 4 == 0 or attempt_num == 1) else "REFINEMENT"
    ...
    if previous_errors and previous_code:
        # Set explore_mode to False when in error fixing mode
        explore_mode = False
    ...
    if feedback and attempt_num > 1:
        ...
        if not explore_mode:
            # MODIFIED: Always use the most recent strategy for refinement
            target_strategy = most_recent_strategy
            
            if target_strategy["description"]:
                # Add important context about the performance
                performance_context = ""
                if int(target_strategy["performance"]) == 1000:
                    performance_context = (
                        "\nIMPORTANT: This strategy performed at exactly €1000, which means it never executed any trades "
                        "or had a net zero impact (we always finish with €1000 if we do nothing). The strategy's decision logic is likely "
...
def extract_clean_response(response_text):
    ...
    if not cleaned_code.startswith("# Strategy:"):
        cleaned_code = extract_strategy_line(cleaned_code) + '\n' + cleaned_code
    
    return cleaned_code

def extract_strategy_line(code_text: str) -> str:
    """
    Extract the strategy line comment from a function's code.
    The strategy line typically starts with '# Strategy:' and can be located
    either above or below the function definition.
    
    Args:
        code_text: String containing the function code
        
    Returns:
        The strategy line if found, otherwise an empty string
    """
    # Split the code into lines
    lines = code_text.strip().split('\n')
    
    # Search for the strategy line
    for line in lines:
        line = line.strip()
        if line.startswith('# Strategy:'):
            return line
    
    return "# Strategy: Generic Trading Strategy - Generated output needed cleaning\n"  # Return empty string if no strategy line found
...
def validate_strategy(code: str, historical_data: list[ExchangeRateItem], fear_greed_data: list[FearGreedItem] = None, debug: bool = False) -> tuple[bool, str, list]:
...
def main():
    ...
    for attempt in range(1, args.attempts + 1):
        ...
        try:
            ...
            valid, message, errors = validate_strategy(code, historical_data, fear_greed_data, debug=True)
