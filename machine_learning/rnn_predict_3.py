...
class RnnPredict:
    def train():
        ...
        csv_path = "files/BTC-USD.csv"
        df = pd.read_csv(csv_path, parse_dates=['Date'])
        df = df.sort_values('Date')
        #df[(df['Date'] > '2013-01-01') & (df['Date'] < '2021-08-31')] doesnt work
        prices = df.Close.values.tolist()[2000:-1]
        dates = pd.to_datetime(df['Date']).dt.date.unique().tolist()[2000:-1]

        price_matrix, _ = DataPrep.price_matrix_creator(prices, dates, seq_len=30) # Creating a matrix using the dataframe
        price_matrix = DataPrep.normalize_windows(price_matrix) # Normalizing its values to fit to RNN
        row, X_train, y_train, X_test, y_test = DataPrep.train_test_split_(price_matrix, train_size=.96) # Applying train-test splitting, also returning the splitting-point

        # LSTM Model parameters, I chose
        batch_size = 8            # Batch size (you may try different values)
        epochs = 80               # Epoch (you may try different values)
